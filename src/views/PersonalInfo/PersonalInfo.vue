<template>
  <div class="wrapper">
    <div class="header">
      <div class="header__info">
        <div class="header__info__user">{{ userName }}</div>
        <div class="header__info__id">ID: {{ myId }}</div>
      </div>

      <img class="header__avatar" src="../../assets/1.gif" />
    </div>

    <div class="list">
      <div class="list__item" @click="handleAddressClick">
        <div class="list__item__icon iconfont">&#xe636;</div>
        <div class="list__item__test">我的地址</div>
        <div class="list__item__arrow iconfont">&#xe743;</div>
      </div>
    </div>

    <div class="logout" @click="handleLogout">退出登陆</div>
  </div>

  <Docker :currentIndex="3" />
</template>

<script>
import { useRouter } from "vue-router";

import { useUserEffect } from "./hooks/useUserEffect";
import Docker from "../../components/Docker.vue";

export default {
  name: "PersonalInfo",
  components: {
    Docker,
  },
  setup() {
    const router = useRouter();

    const { myId, userName } = useUserEffect();

    const handleAddressClick = () => {
      router.push({ name: "MyAddressList" });
    };

    const handleLogout = () => {
      localStorage.removeItem("isLogin");
      router.replace({ name: "Login" });
    };

    return {
      myId,
      userName,

      handleAddressClick,

      handleLogout,
    };
  },
};
</script>

<style lang="scss" scoped>
@import "../../style/viriables.scss";

.wrapper {
  position: absolute;
  left: 0;
  top: 0;
  bottom: 0.5rem;
  right: 0;
  background: $dark-bgColor;
}

.header {
  position: relative;
  height: 1.5rem;
  background-image: linear-gradient(239deg, #fc67a5 0%, pink 100%);
  padding-top: 0.4rem;
  &__info {
    height: 0.66rem;
    background: #fff;
    border-radius: 0.08rem;
    margin: 0.3rem 0.18rem 0 0.18rem;
    padding-top: 0.46rem;
    &__user {
      font-size: 0.24rem;
      color: #262628;
      text-align: center;
      line-height: 0.36rem;
    }
    &__id {
      font-size: 0.14rem;
      color: #c1c0c9;
      text-align: center;
      margin: 0.04rem;
    }
  }
  &__avatar {
    position: absolute;
    left: 50%;
    top: 0.2rem;
    transform: translateX(-50%);
    width: 0.94rem;
    height: 0.94rem;
    border-radius: 50%;
  }
}

.list {
  margin: 0.16rem 0.18rem 0 0.18rem;
  background: $bgColor;
  box-shadow: 0 0.08rem 0.16rem 0 rgb(0 0 0 / 8%);
  border-radius: 0.08rem;
  &__item {
    display: flex;
    line-height: 0.22rem;
    font-size: 0.14rem;
    color: #262626;
    padding: 0.2rem;
    &__icon {
      width: 0.22rem;
      height: 0.22rem;
      /* background: #fc67a5; */
      border-radius: 0.08rem;
      text-align: center;
      margin-right: 0.12rem;
    }
    &__test {
      flex: 1;
    }
    &__arrow {
      width: 0.2rem;
      font-weight: bold;
      color: #c2c4ca;
      font-size: 0.18rem;
    }
  }
}

.logout {
  line-height: 0.32rem;
  background: pink;
  color: $bgColor;
  margin: 0.18rem;
  text-align: center;
  border-radius: 0.04rem;
}
</style>